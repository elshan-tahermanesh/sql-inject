...
import { ArrowLeft, Globe, Terminal, Play, RotateCcw, ShieldAlert, Server, Wifi, Cpu, CheckCircle2, XCircle, Code, Maximize2, Minimize2 } from 'lucide-react';
...
export default function RemoteLoginBypass() {
  ...
  const presets = [
    { name: "Golimar Store Login", url: "http://127.0.0.1:2000/login", payload: "admin ' OR 1=1 --", desc: "Local authentication bypass demonstration" }
  ];
  ...
  const handleSimulate = async () => {
    ...
    try {
      setCurrentStep(0);
      currentPhaseRef.current = 0;
      const res = await executeAttack('login-bypass', targetUrl, payload);
      ...
      if (res && res.success) {
        ...
        const parsedLogs = lines.map(line => {
          ...
          else if (line.includes('[SUCCESS]') || line.includes('SUCCESS!') || line.includes('SUCCESS') || line.includes('[CONNECT] Connection successfully established.') || line.includes('[DNS] Resolved hostname to target node IP:') || line.includes('[DNS] Resolved hostname to server IP:')) type = 'success';
          else if (line.includes('[WARNING]') || line.includes('[HTTP]') || line.includes('[PROBE]') || line.includes('[PAYLOAD]') || line.includes('[RESPONSE]') || line.includes('[INFO]') || line.includes('[DNS]') || line.includes('[SECURITY_SIGNAL]')) type = 'warning';
        ...
        const isSuccess = res.hasOwnProperty('authentication_bypassed') ? res.authentication_bypassed : output.includes("SUCCESS!");

        if (isSuccess && !hasError) {
          setSimResult('success');
          saveTestLog('Login Bypass', payload, 'EXPLOITED', `SELECT * FROM users WHERE email = '${payload}' AND password = 'test'`);
        } else {
          setSimResult('failed');
          saveTestLog('Login Bypass', payload, 'BLOCKED', `SELECT * FROM users WHERE email = '${payload}' AND password = 'test'`);
        }
      } else {
        setCurrentStep(0);
        currentPhaseRef.current = 0;
  ...
  return (
    <div className="space-y-8 select-none w-full">
      {/* Back Link */}
      <Link to="/instructions/login-bypass" className="inline-flex items-center gap-2 text-xs font-mono text-cyber-dim hover:text-cyber-blue transition-colors">
      ...
      <div className="flex flex-col gap-2 border-b border-cyber-border/40 pb-6">
        <h1 className="text-3xl font-extrabold text-cyber-title tracking-wide uppercase">
          Login Bypass SQL <span className="text-cyber-blue">Injection</span>
        </h1>
        <p className="text-cyber-text text-sm text-justify w-full leading-relaxed font-medium">
          Test the security of your authentication endpoints. Supply target coordinates, submit login bypass payloads, and analyze the real HTTP communication status and authentication bypass results.
        </p>
      </div>
      ...
      <div className="bg-cyber-card border border-cyber-border rounded-xl p-6 relative overflow-hidden space-y-5 shadow-lg w-full">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                disabled={simulating}
                placeholder="http://127.0.0.1:2000/login"
        ...
        <div className="flex gap-3 pt-2">
          ...
          >
            ...
            {simulating ? "Executing..." : "Execute Injection"}
          </button>
          <button
            onClick={handleClear}
            disabled={simulating}
            ...
            >
              <div className="w-full space-y-1.5">
                <div className="flex justify-between items-center gap-2">
                  ...
                  <span className="text-[8px] text-[#ff3a3a] border border-[#ff3a3a]/30 px-1.5 py-0.25 rounded bg-[#ff3a3a]/5 shrink-0 uppercase tracking-wider">
                    vulnerable
                  </span>
                </div>
      ...
      <div className="bg-cyber-card border border-cyber-border rounded-xl p-5 font-mono text-xs space-y-4 shadow-lg w-full">
        ...
        <div className="grid grid-cols-5 gap-2 text-center text-[9px] font-bold tracking-wider text-cyber-dim">
          ...
          <div className={currentStep >= 3 ? "text-cyber-blue" : ""}>3. PROBE</div>
      ...
      >
        ...
        <div ref={scrollContainerRef} className="flex-1 min-h-0 p-4 overflow-y-auto font-mono text-[11px] leading-relaxed space-y-2 custom-scrollbar">
          {consoleLogs.length === 0 ? (
            <div className="text-cyber-dim/40 h-full flex flex-col items-center justify-center text-center p-8 select-none">
              ...
              <span>Scanner Console Idle.</span>
              <span className="text-[10px] block mt-1">Configure Target parameters and trigger "Execute Bypass" to launch exploit simulation pipeline.</span>
            </div>
          ) : (
        ...
        {/* Result Overlay Block */}
        {simResult && (
          <div className="border-t border-cyber-border/70 p-4 bg-cyber-card/90 backdrop-blur-sm shrink-0 animate-fade-in">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className={`text-xs font-bold uppercase tracking-wider block ${
                    simResult === 'success' ? 'text-cyber-green' : 'text-cyber-red'
                  }`}>
                    {simResult === 'success' ? "EXPLOIT MATCH SUCCESS" : "EXPLOIT VERIFICATION FAILED"}
                  </span>
              ...
              <div className="text-[10px] font-mono font-bold">
                {simResult === 'success' ? (
                  <span className="text-cyber-green border border-cyber-green/30 bg-cyber-green/5 px-2.5 py-1.5 rounded uppercase">
                    Vulnerable
                  </span>
                ) : (
                  <span className="text-cyber-dim border border-cyber-border px-2.5 py-1.5 rounded uppercase">
                    Not Vulnerable
                  </span>
                )}
              </div>
            ...
            {simResult === 'success' ? (
              <div className="mt-3 bg-cyber-green/5 border border-cyber-green/20 rounded-lg p-3 text-[11px] font-mono text-cyber-green leading-normal">
                <span className="font-bold uppercase block mb-1">Why this injection bypass succeeded:</span>
                The target system interpolated the SQL parameter input directly into the database query execution scope. 
                Adding <code className="bg-cyber-black text-[#ffbd2e] px-1 py-0.25 rounded font-mono font-bold">{payload}</code> successfully closed the username quotes and neutralized the trailing password verification conditional blocks via single-line comments.
              </div>
            ) : (
              <div className="mt-3 bg-cyber-red/5 border border-cyber-red/20 rounded-lg p-3 text-[11px] font-mono text-cyber-dim leading-normal">
                <span className="font-bold uppercase text-cyber-red block mb-1">Why this query failed:</span>
                The SQL statement did not successfully short-circuit the database parser logic. Ensure you are inserting matching quotes, comments (like <code className="bg-cyber-black text-[#ffbd2e] px-1 py-0.25 rounded font-mono">--</code> or <code className="bg-cyber-black text-[#ffbd2e] px-1 py-0.25 rounded font-mono">#</code>), or tautology logic structures.
              </div>
            )}
          </div>
        )}
