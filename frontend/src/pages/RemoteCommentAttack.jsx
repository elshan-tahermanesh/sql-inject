...
import { ArrowLeft, Globe, Terminal, Play, RotateCcw, ShieldAlert, Server, Wifi, Cpu, CheckCircle2, XCircle, Code, Lock, Maximize2, Minimize2 } from 'lucide-react';
...
export default function RemoteCommentAttack() {
  const [targetUrl, setTargetUrl] = useState('http://127.0.0.1:2000/login');
  const [payload, setPayload] = useState("admin@example.com' --");
  ...
  const presets = [
    { name: "Golimar Login", url: "http://127.0.0.1:2000/login", payload: "admin@example.com' --", password: "test", desc: "Comment-based authentication bypass demonstration." }
  ];
  ...
  const handleSimulate = async () => {
    ...
    try {
      setCurrentStep(0);
      currentPhaseRef.current = 0;
      const res = await executeAttack('comment', targetUrl, payload, { password });
      ...
      if (res && res.success) {
        ...
        const parsedLogs = lines.map(line => {
          ...
          else if (line.includes('[SUCCESS]') || line.includes('SUCCESS!') || line.includes('SUCCESS') || line.includes('[CONNECT] Connection successfully established.') || line.includes('[DNS] Resolved hostname to server host IP:')) type = 'success';
        ...
        const isSuccess = res.hasOwnProperty('comment_injection') ? res.comment_injection : output.includes("SUCCESS!");
        if (isSuccess && !hasError) {
          setSimResult('success');
          saveTestLog('Comment Attack', payload, 'EXPLOITED', `SELECT * FROM users WHERE email = '${payload}' AND password = '${password}'`);
        } else {
          setSimResult('failed');
          saveTestLog('Comment Attack', payload, 'BLOCKED', `SELECT * FROM users WHERE email = '${payload}' AND password = '${password}'`);
        }
      } else {
        setCurrentStep(0);
        currentPhaseRef.current = 0;
  ...
  return (
    <div className="space-y-8 select-none w-full">
      {/* Back Link */}
      <Link to="/instructions/comment-attack" className="inline-flex items-center gap-2 text-xs font-mono text-cyber-dim hover:text-cyber-blue transition-colors">
      ...
      <div className="flex flex-col gap-2 border-b border-cyber-border/40 pb-6 w-full">
        <h1 className="text-3xl font-extrabold text-cyber-title tracking-wide uppercase">
          Comment-Based <span className="text-cyber-blue">SQL Injection</span>
        </h1>
        <p className="text-cyber-text text-sm text-justify w-full leading-relaxed font-medium">
          This demonstration performs a real HTTP request against the intentionally vulnerable Golimar login page. The comment operator instructs the database engine to ignore the password verification logic, bypassing authentication.
        </p>
      </div>
      ...
      <div className="bg-cyber-card border border-cyber-border rounded-xl p-6 relative overflow-hidden space-y-5 shadow-lg w-full">
        ...
        <div className="space-y-4 font-mono text-xs">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-cyber-dim/60" />
              <input
                type="text"
                value={password}
                ...
                disabled={simulating}
                placeholder="test"
                className="w-full bg-cyber-input border border-cyber-border rounded-lg pl-10 pr-4 py-2.5 text-cyber-text placeholder-cyber-dim/40 focus:outline-none focus:border-cyber-blue focus:ring-1 focus:ring-cyber-blue/20 transition-all font-bold font-mono tracking-wider"
              />
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-3 pt-2">
          ...
          >
            ...
            {simulating ? "Executing Bypasses..." : "Execute Injection"}
          </button>
          <button
            onClick={handleClear}
            disabled={simulating}
          ...
          {presets.map((preset, idx) => (
            ...
            >
              <div className="w-full space-y-1.5">
                ...
                <div className="text-[10px] text-cyber-dim/80 leading-relaxed">
                  {preset.desc}
                </div>
              </div>
      ...
      <div className="w-full space-y-6 pt-4 flex flex-col items-center">
        ...
        <div className="bg-cyber-card border border-cyber-border rounded-xl p-5 font-mono text-xs space-y-4 shadow-lg w-full">
          ...
          <div className="grid grid-cols-5 gap-2 text-center text-[9px] font-bold tracking-wider text-cyber-dim">
            ...
            <div className={currentStep >= 4 ? "text-cyber-blue" : ""}>4. COMMENT</div>
        ...
        >
          ...
          <div ref={scrollContainerRef} className="flex-1 min-h-0 p-4 overflow-y-auto font-mono text-[11px] leading-relaxed space-y-2 custom-scrollbar">
            {consoleLogs.length === 0 ? (
              <div className="text-cyber-dim/40 h-full flex flex-col items-center justify-center text-center p-8 select-none">
                ...
                <span>Comment-Based SQL Injection terminal is ready.</span>
                <span className="text-[10px] block mt-1">Configure Target details and execute comment payloads to bypass authentication check.</span>
              </div>
            ) : (
          ...
          {/* Session Bypass Dashboard */}
          {simResult === 'success' && (
            <div className="border-t border-cyber-border/70 p-4 bg-cyber-card/95 backdrop-blur-sm shrink-0 animate-fade-in space-y-3 font-mono text-[11px] text-cyber-text">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold text-cyber-green uppercase tracking-wider flex items-center gap-1.5">
                  <Lock className="w-3.5 h-3.5 text-cyber-green animate-pulse" />
                  Bypassed Session Authenticated Dashboard
                </span>
                <span className="text-[9px] text-cyber-green border border-cyber-green/40 bg-cyber-green/10 px-2 py-0.5 rounded font-bold uppercase tracking-wider">
                  SESSION_CREATED
                </span>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="bg-cyber-black/40 border border-cyber-border/40 rounded-lg p-2.5 space-y-1">
                  <div><span className="text-cyber-dim">Bypassed User:</span> <span className="text-cyber-title font-semibold">admin@example.com</span></div>
                  <div><span className="text-cyber-dim">Authorization:</span> <span className="text-cyber-green">Super Administrator</span></div>
                  <div><span className="text-cyber-dim">Comment Style:</span> <span className="text-cyber-blue-light">{payload.includes("--") ? "Double-dash comment (--)" : "Comment Token"}</span></div>
                </div>

                <div className="bg-cyber-black/40 border border-cyber-border/40 rounded-lg p-2.5 text-xs text-cyber-dim leading-snug">
                  Comment Injection terminates the remainder of the SQL statement using the SQL comment operator. Instead of injecting a Boolean expression, the attacker removes the password comparison entirely.
                </div>
              </div>
            </div>
          )}
